...
impl PipeWireCapture {
    ...
    pub fn capture_with_streaming_preview(
        ...
        let mut props = properties! {
            ...
            *pw::keys::APP_ID => "io.github.davidkodar.Spevox",
            *pw::keys::NODE_NAME => "spevox-capture",
            *pw::keys::NODE_DESCRIPTION => "Spevox microphone capture",
        };
        if let Some(target) = target {
            props.insert(*pw::keys::TARGET_OBJECT, target);
        }
        let stream = pw::stream::StreamBox::new(&core, "spevox-capture", props)
            .map_err(AudioCaptureError::pw)?;
        ...
        let _listener = stream
            ...
            .process(move |stream, _| {
                ...
                if let Some(bytes) = data.data() {
                    ...
                    drop(output);
                    // Spevox's non-native streaming engines refresh near
