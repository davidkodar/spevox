...
use spevox_audio::{AudioBuffer, CaptureStopToken, PipeWireCapture};
use spevox_delivery::ClipboardDelivery;
use spevox_portal::{
    GlobalShortcutBinding, GlobalShortcutConfig, GlobalShortcutEvent, TextInputSession,
};
use spevox_transcription::{TranscriptionConfig, WhisperTranscriber};
use tokio::sync::mpsc;
...
async fn main() {
        ...
        .is_some_and(|argument| argument == "--diagnose-clipboard")
    {
        let text = arguments
            .get(1)
            .map_or("Spevox clipboard diagnostic", String::as_str);
    ...
    println!("Spevox diagnostics. Use --help to list available checks.");
}
