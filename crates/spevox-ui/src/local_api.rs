...
pub fn token_path() -> PathBuf {
    ...
    let current = base.join("spevox").join("local-api.token");
    let legacy = base.join("fluidvoice").join("local-api.token");
...
mod tests {
    ...
    fn generated_token_is_strong_and_reusable() {
        let path = std::env::temp_dir().join(format!("spevox-api-token-{}", std::process::id()));
    ...
    fn health_and_status_require_bearer_token() {
        let path = std::env::temp_dir().join(format!("spevox-api-http-{}", std::process::id()));
    ...
    fn authenticated_toggle_dispatches_exactly_one_action() {
        let path = std::env::temp_dir().join(format!("spevox-api-toggle-{}", std::process::id()));
    ...
    fn missing_or_empty_token_fails_closed() {
        let path =
            std::env::temp_dir().join(format!("spevox-api-missing-token-{}", std::process::id()));
        fs::remove_file(&path).ok();
