...
use spevox_audio::{AudioBuffer, StreamingAsrConverter};
...
pub const SORTFORMER_V2: Model = Model {
    ...
    url: "https://github.com/davidkodar/spevox/releases/download/models-sortformer-v2-q8_0/sortformer-v2-q8_0.gguf",
    realtime: true,
};
        ...
        std::env::var_os("XDG_DATA_HOME").map_or_else(
            || {
                PathBuf::from(std::env::var_os("HOME").expect(
                    "Spevox requires HOME or XDG_DATA_HOME; refusing shared temporary storage",
                ))
                .join(".local/share")
            },
            PathBuf::from,
        );
    let current = base.join("spevox");
    let legacy = base.join("fluidvoice");
    let current_has_data = fs::read_dir(&current)
        .ok()
        .is_some_and(|mut entries| entries.next().is_some());
    if !current_has_data && legacy.exists() {
        legacy
    } else {
        current
    }
...
impl Supervisor {
    ...
    pub fn ensure_ready(&mut self, backend: Backend, model: Model) -> Result<(), String> {
        ...
        command
            ...
            .arg(thread_count);
        // Spevox uses Nemotron for completed dictation rather than a
        ...
        for model in MODELS {
            ...
            assert!(
                model.url.starts_with("https://huggingface.co/")
                    || model
                        .url
                        .starts_with("https://github.com/davidkodar/spevox/")
            );
        }
