...
pub(super) fn data_directory() -> PathBuf {
    let base = if let Some(directory) = std::env::var_os("XDG_DATA_HOME") {
        PathBuf::from(directory)
    } else {
        PathBuf::from(
            std::env::var_os("HOME")
                .expect("Spevox requires HOME or XDG_DATA_HOME; refusing shared temporary storage"),
        )
        .join(".local/share")
    };
    prefer_new_or_legacy_directory(&base, "spevox", "fluidvoice")
}

fn prefer_new_or_legacy_directory(base: &std::path::Path, current: &str, legacy: &str) -> PathBuf {
...
pub(super) fn save_audio_history(
    audio: &spevox_audio::MonoAudioBuffer,
    budget_bytes: u64,
