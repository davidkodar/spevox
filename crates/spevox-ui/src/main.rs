...
fn main() -> std::process::ExitCode {
    ...
    let Some(application_ref) = application.as_ref() else {
        eprintln!("Spevox could not initialize the Qt application.");
        return std::process::ExitCode::FAILURE;
    };
    if !application::is_primary_instance(application_ref) {
        eprintln!("Spevox is already running; requested its settings window.");
    ...
    if let Some(mut engine) = engine.as_mut() {
        ...
        engine.as_mut().load(&QUrl::from(
            "qrc:/qt/qml/io/github/davidkodar/Spevox/qml/Main.qml",
        ));
    }
